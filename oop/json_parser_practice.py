...
class GetInfoFromCalldetail(object):
    """
    class for call_detail_str with a metaclass named by StructureAddMetaClass. 
    """
    
    def __new__(cls, call_detail_str):  
        """
        ...
        for name_str, type_class, pLevel_str in structureList:
            setattr(cls, name_str, type(
                ...
                "name":name_str,
                "type":type_class,
                "pLevel":None if pLevel_str is None else getattr(cls, pLevel_str),
                "context":None,
                })()
                )
