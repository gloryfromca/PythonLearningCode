...
<time datetime="2017-01-12T00:00:00+00:00">12 Jan. &ndash; 15 Jan. <span class="say-no-more"> 2017</span></time>
                    ...
                    <li>
                        <h3 class="event-title"><a href="/events/python-events/474/">PyCon Pune 2017</a></h3>
                        <p>
                            
                            
<time datetime="2017-02-16T00:00:00+00:00">16 Feb. &ndash; 20 Feb. <span class="say-no-more"> 2017</span></time>

                            

                            
                            <span class="event-location">COEP, Pune, India</span>
                            
                        </p>
                    </li>
                
                    <li>
                        <h3 class="event-title"><a href="/events/python-events/479/">Iran Pycon &quot;IrPycon&quot;</a></h3>
                        <p>
                            
                            
<time datetime="2017-02-17T00:00:00+00:00">17 Feb. &ndash; 18 Feb. <span class="say-no-more"> 2017</span></time>

                            

                            
                            <span class="event-location">Elm va San&#39;at University, Tehran, Iran</span>
...
class MyHTMLParser(HTMLParser):
    test=''
    def __init__(self):
        self.useful_content=False
        self.l=[]
        self.s={}
        self.a=''
        self.exe=True
        HTMLParser.__init__(self)#初始化方法，除了上面几个赋值之外，还包括用HTMLParser的初始化方法

    
    def handle_starttag(self, tag, attrs):
        if tag=='ul' and attrs[0][1]=="list-recent-events menu" :
            self.useful_content=True
            # print(tag)
               
            self.test='sss'#在类内定义实例属性   
        
            
        if tag=='h3'and self.useful_content:
            self.a=attrs[0][1]
            self.exe=True
            # print(type(attrs))
            # print(type(attrs[0]))
            # print(type(attrs[0][1]))#attrs是一个tuple为元素的list
            # print(type(tag))
        elif tag=='time'and self.useful_content:
            self.a=attrs[0][0]
            self.exe=True
        elif tag=='span'  and self.useful_content:
            self.a=attrs[0][1]
            self.exe=True
        else:
            self.exe=False
    ...
    def handle_data(self, data):
        if self.useful_content==True and self.exe:
            self.exe=False
            self.s[self.a]=data
    ...
    def handle_endtag(self, tag) :
        if tag=='ul' and self.useful_content==True:
            self.useful_content=False
        if tag=='li' and self.useful_content==True:
            self.l.append(self.s)#放在list中的不是数值而是self.s，所以左后放进去的都是相同的数值
            self.s={}
            #重置self.s
parser = MyHTMLParser()
parser.feed(r)
# print(parser.l)
print('--------------------')
for item in parser.l:
    for it in item:
        print(it,':',item[it])
    print('\n')
print('--------------------')
print(parser.test)#可以直接在类内定义实例属性
...
class MyHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if tag=='ul' and attrs[0][1]=="list-recent-events menu" :
            ...
            # print(tag)
            self.test='sss'#在类内定义实例属性
            test='aaa'   #只是函数内的定义，不是类的属性。
        if tag=='li'and self.useful_content==True:
            self.s={}#重新赋值，地址改变，原有的指向还是之前的值

        if len(attrs)>0:
            
            if tag=='a' and self.useful_content==True:
                self.a='event-title'
                self.exe=True
                # print(type(attrs))
                # print(type(attrs[0]))
                # print(type(attrs[0][1]))#attrs是一个tuple为元素的list
                # print(type(tag))
            elif attrs[0][0]=='datetime'and self.useful_content==True:
                self.a=attrs[0][0]
                self.exe=True
            elif attrs[0][1]=='event-location'and self.useful_content==True:
                self.a=attrs[0][1]
                self.exe=True
            else:
                self.exe=False
    ...
    def handle_data(self, data):    
        if self.useful_content==True and self.exe==True:
            self.exe=False
            self.s[self.a]=data
